...
async def test_suggest_sparse_when_no_data():
    ...
    with patch.object(tr, "current_user") as cu, patch.object(tr, "get_app_state") as gas, patch.object(
        tr.db, "connect"
    ) as conn_fn, patch.object(tr.db, "init_schema"), patch.object(
        tr.db, "get_latest_training_readiness", return_value=None
    ), patch.object(tr.db, "get_latest_training_status", return_value=None), patch.object(
        tr.db, "list_active_goals", return_value=[]
    ), patch.object(tr.db, "fetch_recent_activities", return_value=[]), patch.object(
        tr, "tool_enabled", return_value=True
